...
##### reading in data #####
RFModel <- read_rds("FittedModels/AnnualCONUSRFModel_oob.rds")
### CONUS AOI
...
set.seed(802)
SnotelCONUS_split <- initial_split(SnotelCONUS_df, prop = 0.75)
...
trainCONUS_OctAprAspect_Nosradtmintmax_x <- SnotelCONUS_train[, c("OctApr_prcpSumCDMSum", "OctApr_tmeanCDMSum", "elevation", "slope", "aspect", "landcover_triclass")]
testCONUS_OctAprAspect_Nosradtmintmax_x <- SnotelCONUS_test[, c("OctApr_prcpSumCDMSum", "OctApr_tmeanCDMSum", "elevation", "slope", "aspect", "landcover_triclass")]
trainCONUS_y <- SnotelCONUS_train$peak_swe
testCONUS_y <- SnotelCONUS_test$peak_swe
...
# ?unify
# UnifiedRF <- unify(RFModel$finalModel, data = trainCONUS_OctAprAspect_Nosradtmintmax_x)
...
#   set.seed(802)
#   AK_Fastshap <- explain(AKRFModel$finalModel, X = AKTrain_OctMayDecFebAspect_Nosrad_x, nsim = 50, pred_wrapper = pfun, newdata = as.data.frame(AKPredictors), adjust = TRUE)
...
# }
# 
# ##### calling function to run treeshap on all years for Alaska #####
...
##### Function to call Fastshap and create SHAP-based classes for CONUS #####
SHAPFunc <- function(year){
  ...
  ### SWE_max predictions
  # print("reading in prediction csv")
  CONUSPredictions <- fread(file = paste0("SHAPAnalysis/Data/SHAP/PredictionCSVs/CONUS/", paste0("CONUS_SWEmaxPredictions", year, ".csv")), data.table = FALSE)
  ### removing xy
  # CONUSPredictions_NoXY <- CONUSPredictions[,3]
  colnames(CONUSPredictions) <- c("x", "y", "peak_swe")
  
  # print("reading in predictor csv")
  CONUSPredictors <- fread(file = paste0("SHAPAnalysis/Data/SHAP/PredictorCSVs/CONUS/", paste0("CONUS_SWEmaxPredictors", year, ".csv")), data.table = FALSE) |>
    select(OctApr_prcpSumCDMSum, OctApr_tmeanCDMSum, elevation, slope, aspect, landcover_triclass)
  ...
  # print("running Fastshap")
  set.seed(802)
  CONUSFastshap <- explain(RFModel$finalModel, X = trainCONUS_OctAprAspect_Nosradtmintmax_x, nsim = 25, pred_wrapper = pfun, newdata = as.data.frame(CONUSPredictors), adjust = TRUE)
  CONUSFastshap_df <- as.data.frame(CONUSFastshap)
  write_rds(CONUSFastshap, paste0("SHAPAnalysis/Data/SHAP/Fastshap/CONUS/", paste0("CONUSSWEmax_SHAPs", year, ".rds")), compress = "gz")
  ...
  plot1 <- sv_dependence(sv, v = vars[1], color_var = vars[1], jitter_width = 0) +
    ggtitle("Oct-Apr Precipitation CDM Sum Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = "Oct-Apr Total Precipitation CDM Sum (mm)") +
    ylab(label = "SHAP Value")
  plot2 <- sv_dependence(sv, v = vars[2], color_var = vars[2], jitter_width = 0) +
    ggtitle("Oct-Apr Mean Temperature CDM Sum Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = expression("Oct-Apr Mean Temp CDM Sum ("*degree*C*")")) +
    ylab(label = "SHAP Value")
  plot3 <- sv_dependence(sv, v = vars[3], color_var = vars[3], jitter_width = 0) +
    ggtitle("Elevation Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = "Elevation (m)") +
    ylab(label = "SHAP Value")
  plot4 <- sv_dependence(sv, v = vars[4], color_var = vars[4], jitter_width = 0) +
    ggtitle("Slope Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = "Slope") +
    ylab(label = "SHAP Value")
  plot5 <- sv_dependence(sv, v = vars[5], color_var = vars[5], jitter_width = 0) +
    ggtitle("Aspect Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = "Aspect") +
    ylab(label = "SHAP Value")
  plot6 <- sv_dependence(sv, v = vars[6], color_var = vars[6], jitter_width = 0) +
    ggtitle("Landcover Feature Dependence", subtitle = paste("CONUS", as.character(year))) + 
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
    xlab(label = "Landcover") +
    ylab(label = "SHAP Value")
  ...
  ### Saving Feature Dependence plots
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[1], "/", paste0("CONUS", year, "_", vars[1],  "_FeatureDependence.png")), plot = plot1)
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[2], "/", paste0("CONUS", year, "_", vars[2], "_FeatureDependence.png")), plot = plot2)
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[3], "/", paste0("CONUS", year, "_", vars[3], "_FeatureDependence.png")), plot = plot3)
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[4], "/", paste0("CONUS", year, "_", vars[4], "_FeatureDependence.png")), plot = plot4)
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[5], "/", paste0("CONUS", year, "_", vars[5], "_FeatureDependence.png")), plot = plot5)
  ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/CONUS/", vars[6], "/", paste0("CONUS", year, "_", vars[6], "_FeatureDependence.png")), plot = plot6)
  
  ### Extracting SHAP values, adding dummy rank columns which will be filled in later
  CONUS_SHAPs <- CONUSFastshap_df |>
  ...
  CONUS_SHAPs$class_top1 <- ranked_data[, 8]
  CONUS_SHAPs$class_top2 <- ranked_data[, 9]
  CONUS_SHAPs$class_top3 <- ranked_data[, 10]
  CONUS_SHAPs$class_top4 <- ranked_data[, 11]
  CONUS_SHAPs$class_top5 <- ranked_data[, 12]
  ...
  ### binding Shapley values to Predicted Peak SWE df to be rasterized and also for kmeans later maybe
  CONUS_SHAPs <- cbind(CONUSPredictions, CONUS_SHAPs)
  
  ### saving SHAPs
  fwrite(CONUS_SHAPs, file = paste0("SHAPAnalysis/Data/SHAP/Fastshap/SHAPs/CONUS/", paste0("CONUS", year, "SHAPs.csv")))
  
  ### reading in prediction raster to use as basis for rasterization
  PredictionRast <- rast(paste0("SHAPAnalysis/Outputs/MapTIFFs/CONUS/", paste0("CONUS_SWEmax", year, ".tif")))
  CONUS_SHAPClasses <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class")
  ### top 1
  CONUS_SHAPClasses_top1 <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class_top1")
  ### top 2
  CONUS_SHAPClasses_top2 <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class_top2")
  ### top 3
  CONUS_SHAPClasses_top3 <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class_top3")
  ### top 4
  CONUS_SHAPClasses_top4 <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class_top4")
  ### top 5
  CONUS_SHAPClasses_top5 <- terra::rasterize(vect(st_as_sf(CONUS_SHAPs, coords = c("x", "y"), crs = crs(CONUS_AOI))), y = PredictionRast, field = "class_top5")
  
  ### Saving Raster
  writeRaster(CONUS_SHAPClasses, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses.tif")), overwrite = TRUE)
  writeRaster(CONUS_SHAPClasses_top1, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses_top1.tif")), overwrite = TRUE)
  writeRaster(CONUS_SHAPClasses_top2, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses_top2.tif")), overwrite = TRUE)
  writeRaster(CONUS_SHAPClasses_top3, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses_top3.tif")), overwrite = TRUE)
  writeRaster(CONUS_SHAPClasses_top4, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses_top4.tif")), overwrite = TRUE)
  writeRaster(CONUS_SHAPClasses_top5, file = paste0(getwd(), "/SHAPAnalysis/Data/SHAP/Classes/CONUS/", paste0("CONUS", year, "SHAPClasses_top5.tif")), overwrite = TRUE)
  ...
  # # print("running Fastshap")
  # set.seed(802)
  # AK_Fastshap <- explain(RFModel$finalModel, X = train_OctAprAspect_Nosradtmintmax_x, nsim = 50, pred_wrapper = pfun, newdata = as.data.frame(AKPredictors), adjust = TRUE)
  # AK_Fastshap_df <- as.data.frame(AK_Fastshap)
  # write_rds(AK_Fastshap, paste0("SHAPAnalysis/Data/SHAP/Fastshap/Alaska/", paste0("AKSWEmax_SHAPs", year, ".rds")), compress = "gz")
  # 
  # ### making plots of feature dependence
  ...
  #   ylab(label = "SHAP Value")
  # plot2 <- sv_dependence(sv, v = vars[2], color_var = vars[2], jitter_width = 0) +
  #   ggtitle("Oct-Apr Mean Temperature CDM Sum Feature Dependence", subtitle = paste("Alaska", as.character(year))) + 
  #   theme_bw() +
  #   theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  #   xlab(label = expression("Oct-Apr Mean Temp CDM Sum ("*degree*C*")")) +
  #   ylab(label = "SHAP Value")
  ...
  # 
  # ### Saving Feature Dependence plots
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[1], "/", paste0("Alaska", year, "_", vars[1], "_FeatureDependence.png")), plot = plot1)
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[2], "/", paste0("Alaska", year, "_", vars[2], "_FeatureDependence.png")), plot = plot2)
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[3], "/", paste0("Alaska", year, "_", vars[3], "_FeatureDependence.png")), plot = plot3)
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[4], "/", paste0("Alaska", year, "_", vars[4], "_FeatureDependence.png")), plot = plot4)
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[5], "/", paste0("Alaska", year, "_", vars[5], "_FeatureDependence.png")), plot = plot5)
  # ggsave(paste0("SHAPAnalysis/Outputs/Plots/Fastshap/FeatureDependence/Alaska/", vars[6], "/", paste0("Alaska", year, "_", vars[6], "_FeatureDependence.png")), plot = plot6)
  # 
  # 
  # ### Extracting SHAP values, adding dummy rank columns which will be filled in later
  # AK_SHAPs <- AK_Fastshap_df |>
  ...
  # AK_SHAPs$class_top1 <- ranked_data[, 8]
  # AK_SHAPs$class_top2 <- ranked_data[, 9]
  # AK_SHAPs$class_top3 <- ranked_data[, 10]
  # AK_SHAPs$class_top4 <- ranked_data[, 11]
  # AK_SHAPs$class_top5 <- ranked_data[, 12]
  # 
  # ### binding Shapley values and initial classes to predicted peak SWE df to be rasterized by class below
  # AK_SHAPs <- cbind(AKPredictions, AK_SHAPs)
  # ### saving SHAPs
...
##### calling function to run Fastshap on all years on CONUS #####
## change number of cores if running on personal computer
CONUSSWE_SHAPs <- pmclapply(X = years, FUN = SHAPFunc, mc.cores = ifelse(length(years) > detectCores(), ceiling(detectCores() / 1.5), length(years)), mc.silent = FALSE, mc.set.seed = FALSE)
# SWE_SHAPs <- pmclapply(X = years, FUN = SHAPFunc, mc.cores = 14, mc.silent = FALSE, mc.set.seed = FALSE)
